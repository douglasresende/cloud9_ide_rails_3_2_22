...
RUN bash -l -c "rvm install ruby-2.1.10 && rvm use 2.1.10 --create"
RUN bash -l -c "NOKOGIRI_USE_SYSTEM_LIBRARIES=1 gem install rack-cache -v 1.9.0"
RUN bash -l -c "NOKOGIRI_USE_SYSTEM_LIBRARIES=1 gem install rack -v 1.6.8"
RUN bash -l -c "NOKOGIRI_USE_SYSTEM_LIBRARIES=1 gem install rails -v 3.2.22"
